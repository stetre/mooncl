#!/usr/bin/env lua
-- Solving a sparse matrix system Ax=b with the Conjugate Gradient method
...
local program = cl.create_program_with_sourcefile(context, "conj_grad.cl")
cl.build_program(program, {device})
local kernel = cl.create_kernel(program, "conj_grad")
...
cl.set_kernel_arg(kernel, 5, num_rows*cl.sizeof('float'))
cl.set_kernel_arg(kernel, 6, rows_buf)
cl.set_kernel_arg(kernel, 7, cols_buf)
cl.set_kernel_arg(kernel, 8, values_buf)
cl.set_kernel_arg(kernel, 9, b_buf)
cl.set_kernel_arg(kernel, 10, result_buf)
